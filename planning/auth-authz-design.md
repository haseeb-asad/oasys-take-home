...
- **Onboarding / invitations (stub):** an `Invitation` record + token; **no email send**. Client self-signup allowed. Provider/org invites a client → pending `Invitation` → on accept, an **`Identity` (+ client `Profile`)** is created; any care-team membership is added **separately** via `Episode.add_member` (membership is an entity of the Episode aggregate, not created by the invite itself). **Bootstrap policy:** an active provider Profile may create/invite into an episode where that provider becomes the initial responsible member; an org_staff Profile with active admin membership in the target org may create/invite for that org (design intent; v1 ships only the provider-create path, so org-admin episode creation is deferred; see the README open questions). After the episode exists, team changes require `MANAGE_TEAM`.
